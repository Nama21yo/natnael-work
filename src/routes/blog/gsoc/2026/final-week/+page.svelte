...
<div class="grid gap-5 lg:grid-cols-[17rem_minmax(0,1fr)_18rem]">
	<aside class="obsidian-panel h-fit lg:sticky lg:top-28">
		...
		<div class="mt-4 space-y-1">
			...
			<a href={`${base}/blog/gsoc/2026/week-13`} class="block rounded-xl px-3 py-2 text-sm font-bold text-muted-foreground transition hover:bg-muted hover:text-foreground">Week 13</a>
			...
			<a href={`${base}/blog/gsoc/2026/final-week`} class="block rounded-xl bg-cyan/15 px-3 py-2 text-sm font-bold text-cyan">Final Week</a>
		</div>
	</aside>
	...
	<article class="obsidian-panel min-h-[42rem]">
		<div class="border-b border-foreground/10 pb-5">
			...
			<h1 class="mt-3 font-mono text-4xl leading-tight font-black tracking-tight">
				GSoC 2026 Final Week: A Fix for extraction-framework, and agentic-amdbpedia Becomes Demo-Ready
			</h1>
			<p class="mt-5 text-base leading-8 text-muted-foreground">
				Coding officially wrapped in Week 14, but two threads were still open: the seven
				extraction-framework bugs filed as issue #845 needed an actual fix, and
				<code>agentic-amdbpedia</code> -- the ontology-mapping assistant itself -- needed to go from "the
				pipeline works" to "a mentor can sit down and use it end to end." This week pushed both to the
				finish line.
			</p>
		...
		<div class="prose-obsidian mt-8 space-y-10">
			<section>
				...
				<p class="mt-5">
					Week 14 called itself the closing chapter, and for the benchmarking work it was. But the
					two most concrete deliverables of the summer -- an upstream framework fix and a working
					product -- were still unfinished. This week was about finishing them properly rather than
					leaving them as findings and prototypes.
				</p>
			</section>
			...
			<section>
				...
				<p class="mt-5">
					The biggest single number in there is bug B: 94.5% of Ethiopian dates in the dump used the
					standard <code>&lt;month&gt; &lt;day&gt; ቀን &lt;year&gt;</code> form, and none of the five date
					regexes admitted it -- so almost every date was silently dropped rather than mis-parsed. Bug
					E is the quiet one: because the Amharic ignore-list replaced the English baseline instead of
					adding to it, 7% of the entire infobox-properties dataset was image filenames and flag references
			...
			<section>
				...
				<p class="mt-4">
					Nothing in that pipeline is a black box by design. Every accepted or rejected mapping is
					captured into an append-only training-example log, written in the same format
					<a
						href="https://github.com/AmharicDBpedia/LLMIntegration"
						target="_blank"
						rel="noreferrer"
			...
			<section>
				<h2 class="font-mono text-2xl font-bold tracking-tight text-foreground">
					Making it feel like a product, not a demo script
				</h2>
				<p class="mt-5">
					The pipeline working was necessary but not sufficient -- the last stretch of this week was
					a deliberate UI/UX pass so someone unfamiliar with the project could actually sit down,
					paste something in, and understand what happened.
				</p>
			...
			<section>
				...
				<p class="mt-5">
					PR #846 is the concrete, upstream-facing outcome of the summer's extraction-framework
					audit -- seven bugs, evidenced with real dump data, now fixed and validated, waiting on
					review. <code>agentic-amdbpedia</code> is the concrete, hands-on-keyboard outcome: a pipeline
					that goes from a pasted infobox or a bare Wikipedia link to a reviewed, published DBpedia mapping,
					with every decision along the way logged for the next round of benchmarking back in LLMIntegration.
					Fourteen weeks built the evidence and the experiments; this one turned the two most load-bearing
					pieces of that work into things someone else can actually pick up and use.
				</p>
			</section>
		</div>
	</article>
